...
from seqspec.seqspec_info import format_info, seqspec_info
...
def test_format_info(dogmaseq_dig_spec: Assay):
    ...
    info = seqspec_info(spec=dogmaseq_dig_spec, key="sequence_spec")
    formatted_info = format_info(info, "sequence_spec", "json")
    sequence_spec = json.loads(formatted_info)
    assert any(read["read_id"] == "rna_R1" for read in sequence_spec)

    info = seqspec_info(spec=dogmaseq_dig_spec, key="library_spec")
    formatted_info = format_info(info, "library_spec", "json")
    library_spec = json.loads(formatted_info)
    assert "rna" in library_spec
    assert any(region["region_id"] == "rna_cell_bc" for region in library_spec["rna"])
